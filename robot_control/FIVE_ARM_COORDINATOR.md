...
推荐启动 CoppeliaSim。先进入任意成员自己 clone 的仓库根目录，然后用仓库脚本
自动打开当前仓库中的 `scenes/five_cr5a_cell.ttt`：
...
不要把 `/你的克隆目录` 或旧文档里的 `/path/to` 原样复制；它表示每个成员自己的
仓库位置。若 CoppeliaSim 不在常见路径，使用
