"""Shared runtime Cartesian primitives for current-scene robot tasks.
...
def build_tip_translation_path(
    ...
    tip: int,
    joints: list[int],
    start_config: Iterable[float],
    target_position: Iterable[float],
    point_count: int,
    label: str,
) -> list[list[float]]:
    """Build a Cartesian path that translates the visible TCP in world space.

    The TCP orientation at ``start_config`` is retained.  This is used for the
    final grasp approach so the robot reaches the stationary workpiece before
    it is parented to the tool.
    """
    start = [float(value) for value in start_config]
    if sim.getSimulationState() == sim.simulation_stopped:
        pose = tip_pose_for_config(sim, tip, joints, start)
    else:
        pose = [float(value) for value in sim.getObjectPose(tip, -1)]
    pose[:3] = [float(value) for value in target_position]
    target = sim.createDummy(0.004)
    sim.setObjectAlias(target, f"Runtime_{label.replace(' ', '_')}")
    sim.setObjectPose(target, -1, pose)
    sim.setObjectInt32Param(target, sim.objintparam_visibility_layer, 0)
    try:
        ...
        path = generate_cartesian_path(
            ...
            target,
            start,
            max(3, int(point_count)),
            label,
        )
        solved = [
            wrap_near(reference, value)
            for reference, value in zip(path[-1], solved)
        ]
        endpoint_gap = max(
            abs(first - second) for first, second in zip(path[-1], solved)
        )
        if endpoint_gap > math.radians(20.0):
            raise RuntimeError(
                f"{label} IK endpoint changed branch by "
                f"{math.degrees(endpoint_gap):.3f} deg"
            )
        if endpoint_gap > math.radians(0.02):
            path = join_paths(path, interpolate_joint_line(path[-1], solved, 11))
        else:
            path[-1] = solved
        if not near(path[0], start, math.radians(0.5)):
            path.insert(0, start)
        else:
            path[0] = start
        if sim.getSimulationState() == sim.simulation_stopped:
            actual_pose = tip_pose_for_config(sim, tip, joints, path[-1])
            position_error = math.sqrt(
                sum(
                    (actual_pose[index] - pose[index]) ** 2
                    for index in range(3)
                )
            )
            if position_error > 0.002:
                raise RuntimeError(
                    f"{label} FK endpoint error is "
...
def build_tip_pose_path(
    ...
    """Build a Cartesian path to an explicit visible-TCP world pose."""
    start = [float(value) for value in start_config]
    pose = [float(value) for value in target_pose]
    if len(pose) != 7:
        raise ValueError(f"{label} target pose must contain seven values")
    target = sim.createDummy(0.004)
    sim.setObjectAlias(target, f"Runtime_{label.replace(' ', '_')}")
    sim.setObjectPose(target, -1, pose)
    sim.setObjectInt32Param(target, sim.objintparam_visibility_layer, 0)
    try:
        ...
        if endpoint_gap > math.radians(0.02):
            path = join_paths(
                path, interpolate_joint_line(path[-1], solved, 11)
            )
        else:
            path[-1] = solved
        if not near(path[0], start, math.radians(0.5)):
        ...
        if sim.getSimulationState() == sim.simulation_stopped:
            ...
            quaternion_dot = min(
                ...
                abs(
                    sum(
                        actual_pose[index] * pose[index]
                        for index in range(3, 7)
                    )
                ),
            )
            orientation_error = 2.0 * math.acos(quaternion_dot)
            if position_error > 0.002 or orientation_error > math.radians(0.5):
                raise RuntimeError(
                    f"{label} FK endpoint error is "
                    f"{position_error * 1000.0:.3f} mm / "
                    f"{math.degrees(orientation_error):.3f} deg"
                )
        return path
    finally:
        sim.removeObjects([target])
...
def build_pick_place_paths(
    ...
    pick_entry_position: Iterable[float] | None = None,
    ...
    try:
        ...
        if pick_entry_position is None:
            initial = interpolate_joint_line([0.0] * 6, pick_app_config, 101)
            entry_to_app = None
        else:
            pick_entry = create_pose_dummy(
                sim,
                f"{prefix}Target_Pick_Entry",
                pick_entry_position,
                pick_orientation_deg,
            )
            temporary.append(pick_entry)
            app_to_entry = generate_cartesian_path(
                sim_ik,
                base,
                tip,
                joints,
                ...
                pick_app_config,
                61,
                "pick APP to overhead entry",
            )
            pick_entry_config = app_to_entry[-1]
            home_to_entry = interpolate_joint_line(
                [0.0] * 6, pick_entry_config, 101
            )
            entry_to_app = list(reversed(app_to_entry))
            initial = home_to_entry
        ...
        approach_descend = (
            pick_descend
            if entry_to_app is None
            else join_paths(entry_to_app, pick_descend)
        )
        return_segments = [
            list(reversed(place_descend)),
            list(reversed(transfer)),
        ]
        if entry_to_app is not None:
            return_segments.append(list(reversed(entry_to_app)))
        return_segments.append(list(reversed(initial)))
        result = {
            "initial_to_pick_app": initial,
            "pick_descend": approach_descend,
            "lift_and_transfer": join_paths(
                list(reversed(pick_descend)), transfer
            ),
            "place_descend": place_descend,
            "return_home": join_paths(*return_segments),
        }
...
class RobotSafetyGuard:
    ...
    def __init__(
        ...
        all_robot_shapes = set(
            sim.getObjectsInTree(robot, sim.object_shape_type, 0)
        ) - self.payload_shapes
        # TCP markers are non-physical calibration/visualization geometry.
        # Several tools place the marker sphere exactly on the workpiece
        # contact plane, so treating it as robot collision geometry produces
        # a false positive after an otherwise clean release.  Real links,
        # fingers, pads, and tool bodies remain in every safety collection.
        tcp_marker_shapes = {
            handle
            for handle in all_robot_shapes
            if sim.getObjectAlias(handle).endswith("_tcp_marker")
        }
        self.robot_shapes = all_robot_shapes - tcp_marker_shapes
        # Without this, shapes removed from the robot collection are found
        # again by the scene-wide environment scan below.
        ignored_environment.update(tcp_marker_shapes)
