...
The controller retains the taught wait posture and dynamically solves the
native screwdriver's APP/TCP/PRESS path for the fixed inspection station.
...
PROTECTED_TARGETS = {
    "R4_SCREW_APP": {
        "position": [0.03, -0.035, 0.499],
        "orientation_euler": [0.0, 0.0, 0.0],
    },
    "R4_SCREW_TCP": {
        "position": [0.03, -0.035, 0.339],
        "orientation_euler": [0.0, 0.0, 0.0],
    },
    "R4_SCREW_PRESS": {
        "position": [0.03, -0.035, 0.309],
        "orientation_euler": [0.0, 0.0, 0.0],
    },
}

RUNTIME_ORIENTATION_DEG = (0.0, 0.0, -60.0)
...
class R4SafetyGuard:
    ...
    def __init__(self, sim: Any, robot: int, tool_shapes: set[int]):
        ...
        inspection_area_marker = sim.getObject(AREAS["INSPECTION_SCREW"])
        ...
        for handle in sim.getObjectsInTree(
            ...
            if (
                ...
                or handle == inspection_area_marker
            ):
                continue
            if (
...
class R4MotionController:
    ...
    def prepare(self, action: str = R4_SCREW_DONE) -> dict[str, Any]:
        ...
        tool = self._discover_native_tool(robot)
        ...
        home_to_wait = [
            ...
            for config in plan["paths"]["home_to_wait"]
        ]
        wait_to_app = _interpolate_joint_line(
            home_to_wait[-1], prepared_paths["initial_to_app"][-1], 121
        )
        prepared_paths["home_to_wait"] = home_to_wait
        prepared_paths["wait_to_app"] = wait_to_app
        prepared_paths["app_to_wait"] = list(reversed(wait_to_app))
    ...
    def execute(self, action: str) -> dict[str, Any]:
        ...
        try:
            ...
            with self.inspection_lock:
                ...
                runner.hold(0.25, "R4 wait-point clearance hold")
                # R5's good-sort return route passes through the inspection
                # shared zone occupied by R4_WAIT_POINT.  The checked plan
                # already contains the collision-validated HOME->WAIT route;
                # reverse it after screw completion so R4 vacates the zone
                # before the scheduler dispatches R5.
                runner.execute_path(
                    "R4 wait_to_home after screw",
                    list(reversed(paths["home_to_wait"])),
                    transfer_speed,
                )
                runner.hold(0.4, "R4 final home hold")

            final_joints = runner.joint_positions()
            expected_final = paths["home_to_wait"][0]
            if not _near(
                final_joints,
                expected_final,
                math.radians(JOINT_TOLERANCE_DEG),
            ):
                raise RuntimeError("R4 did not return home after screw")
            result = {
                "action": action,
