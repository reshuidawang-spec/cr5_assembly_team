...
# The wide jaw extends about 34 mm below its TCP.  A 202 mm TCP height puts
# the rubber pads on the upper side walls while retaining clearance from the
# 155 mm-high supply trays.
BOX_GRASP_POSITION = (-1.86, 0.22, 0.202)
TERMINAL_GRASP_POSITION = (-1.82, -0.02, 0.202)
BOX_ASSEMBLY_POSITION = (-1.078563, 0.120898, 0.21946)
# Keep 1.5 mm insertion clearance for the wide R1 jaw.  R3's downstream
# assembly preflight accepts <=2 mm and the terminal remains seated visually.
TERMINAL_ASSEMBLY_POSITION = (-1.05397, 0.086735, 0.273073)
...
class R1SafetyGuard:
    ...
    def __init__(
        ...
        self.environment = _EnvironmentCollisionProbe(
            sim, robot, payload, allowed_payload_contacts
        )
...
class _SmoothRunner:
    def __init__(
        ...
        box: int,
        terminal: int,
        box_allowed_contacts: Optional[set[int]],
        terminal_allowed_contacts: Optional[set[int]],
        ...
        self.guards = {
            ...
            box: R1SafetyGuard(
                ...
                robot,
                box,
                allowed_payload_contacts=box_allowed_contacts,
            ),
            terminal: R1SafetyGuard(
                self.sim,
                robot,
                terminal,
                allowed_payload_contacts=terminal_allowed_contacts,
            ),
        }
    ...
    def attach_visual_payload(
        ...
        self.step(f"{label} rigid attachment", force_collision=True)
...
class R1MotionController:
    ...
    def prepare(self, action: str = R1_BOX_PLACED) -> dict[str, Any]:
        ...
        tip = find_unique_alias(sim, robot, ROBOT_TIPS["R1"])
        sim_ik = client.require("simIK")
        self._prepared_grasp_paths = {
            "box": build_tip_translation_path(
                sim,
                sim_ik,
                robot,
                tip,
                joints,
                plan["paths"]["box_descend"][-1],
                BOX_GRASP_POSITION,
                31,
                "R1_box_grasp_contact",
            ),
            "terminal": build_tip_translation_path(
                sim,
                sim_ik,
                robot,
                tip,
                joints,
                plan["paths"]["terminal_descend"][-1],
                TERMINAL_GRASP_POSITION,
                31,
                "R1_terminal_grasp_contact",
            ),
        }
        ...
        return {
            ...
            "grasp_contact_points": {
                name: len(path)
                for name, path in self._prepared_grasp_paths.items()
            },
        }
    ...
    def _release_alignment_path(
        ...
        target_tip = [
            current_tip[index] + target_position[index] - current_payload[index]
    ...
    def execute(self, action: str) -> dict[str, Any]:
        ...
        try:
            ...
            runner = _SmoothRunner(
                ...
                | set(
                    sim.getObjectsInTree(
                        self.bridge.get_object_handle("CONTROL_MODULE_SUPPLY"),
                        sim.object_shape_type,
                        0,
                    )
                )
            ...
            if action in {R1_BOX_PLACED, R1_COMPLETE_CYCLE}:
                ...
                box_grasp = self._prepared_grasp_paths["box"]
                # During the final approach, contact between the visible jaw
                # pads and this specific workpiece is intentional.  The
                # payload guard still checks the rest of the arm and scene.
                runner.set_payload(box)
                runner.execute_path(
                    "R1 box final contact approach", box_grasp, descent_speed
                )
                if not self.bridge.set_gripper_gap("R1", 0.150):
                    raise RuntimeError(
                ...
                runner.execute_path(
                    "R1 lift box from supply",
                    list(reversed(box_grasp)),
                    descent_speed,
                )
                with self.assembly_lock:
                    ...
                    box_release = self._release_alignment_path(
                        robot,
                        tip,
                        joints,
                        box,
                        BOX_ASSEMBLY_POSITION,
                        "R1_box_release_alignment",
                    )
                    runner.execute_path(
                        "R1 lower box to fixture", box_release, descent_speed
                    )
                    if not self.bridge.set_gripper("R1", True):
                        raise RuntimeError(
                            self.bridge.last_error or "cannot open R1 gripper"
                    ...
                    runner.execute_path(
                        "R1 clear released box",
                        list(reversed(box_release)),
                        descent_speed,
                    )
                    runner.execute_path(
                        "box_retreat_and_terminal_approach",
            ...
            if action in {R1_TERMINAL_PLACED, R1_COMPLETE_CYCLE}:
                ...
                terminal_grasp = self._prepared_grasp_paths["terminal"]
                runner.set_payload(terminal)
                runner.execute_path(
                    "R1 terminal final contact approach",
                    terminal_grasp,
                    descent_speed,
                )
                if not self.bridge.set_gripper_gap("R1", 0.030):
                    raise RuntimeError(
                        self.bridge.last_error or "cannot close R1 gripper"
                    )
                runner.hold(0.8, "close gripper")
                ...
                runner.execute_path(
                    "R1 lift terminal from supply",
                    list(reversed(terminal_grasp)),
                    descent_speed,
                )
                ...
                with self.assembly_lock:
                    ...
                    terminal_release = self._release_alignment_path(
                        robot,
                        tip,
                        joints,
                        terminal,
                        TERMINAL_ASSEMBLY_POSITION,
                        "R1_terminal_release_alignment",
                    )
                    runner.execute_path(
                        "R1 lower terminal into box",
                        terminal_release,
                        descent_speed,
                    )
                    # The terminal is released while the jaws are still
                    # inside the box.  Opening directly to the 170 mm home
                    # gap makes the rubber pads strike the box walls.  Add
                    # only 4 mm of workpiece clearance here, withdraw along
                    # the Cartesian insertion path, then fully open above
                    # the box.
                    if not self.bridge.set_gripper_gap("R1", 0.038):
                        raise RuntimeError(
                            self.bridge.last_error
                            or "cannot release terminal from R1 gripper"
                        )
                    runner.hold(0.4, "release terminal inside box")
                    self._freeze_gripper()
                    ...
                    attached_payload = None
                    runner.execute_path(
                        "R1 clear released terminal",
                        list(reversed(terminal_release)),
                        descent_speed,
                    )
                    if not self.bridge.set_gripper("R1", True):
                        raise RuntimeError(
                            self.bridge.last_error
                            or "cannot fully open R1 gripper above box"
                        )
                    runner.hold(0.8, "fully open R1 above box")
                    self._freeze_gripper()
                    runner.execute_path(
